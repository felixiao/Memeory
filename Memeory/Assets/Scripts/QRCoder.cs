    ...
    public class QRCodeGenerator
    {
        ...
        private static class ModulePlacer
        {
            ...
            public static void PlaceFormat(ref QRCode qrCode, string formatStr)
            {
                ...
                for (int i = 0; i < 15; i++)
                {
                    var p1 = new Vector2(modules[i, 0], modules[i, 1]);
                    var p2 = new Vector2(modules[i, 2], modules[i, 3]);
                    qrCode.ModuleMatrix[(int)p1.y][(int)p1.x] = fStr[i] == '1' ? true : false;
                    qrCode.ModuleMatrix[(int)p2.y][(int)p2.x] = fStr[i] == '1' ? true : false;
                }
            }
            ...
            public static void PlaceAlignmentPatterns(ref QRCode qrCode, List<Vector2> alignmentPatternLocations, ref List<Rect> blockedModules)
            {
                foreach (var loc in alignmentPatternLocations)
                {
                    ...
                    for (int x = 0; x < 5; x++)
                    {
                        for (int y = 0; y < 5; y++)
                        {
                            ...
                            {
                                qrCode.ModuleMatrix[(int)loc.y + y][(int)loc.x + x] = true;
                            }
                        }
                    }
                ...
                for (int x = 0; x < 7; x++)
                {
                    if (alignmentPatternBaseValues[i + x] != 0)
                    {
                        for (int y = 0; y < 7; y++)
                        {
                            ...
                            {
                                Vector2 p = new Vector2(alignmentPatternBaseValues[i + x] - 2, alignmentPatternBaseValues[i + y] - 2);
        ...
        {
            public int Version;
            public List<Vector2> PatternPositions;
        }
        ...
        public class ColorConverter
        {
            ...
            public static Color FromHTML(string htmlColorHex, ColorEncoding colorEncoding = ColorEncoding.RGB)
            {
                if (htmlColorHex.Contains('#'))
                {
                    htmlColorHex = htmlColorHex.Trim('#');
                }
                byte r=255;
                byte g=255;
                byte b=255;
                ...
                switch (colorEncoding)
                {
                    case ColorEncoding.RGB:
                        r = byte.Parse(htmlColorHex.Substring(0, 2), System.Globalization.NumberStyles.HexNumber);
                        g = byte.Parse(htmlColorHex.Substring(2, 4), System.Globalization.NumberStyles.HexNumber);
                        b = byte.Parse(htmlColorHex.Substring(4, 6), System.Globalization.NumberStyles.HexNumber);
                        a = 255;
                        break;
                    case ColorEncoding.ARGB:
                        a = byte.Parse(htmlColorHex.Substring(0, 2), System.Globalization.NumberStyles.HexNumber);
                        r = byte.Parse(htmlColorHex.Substring(2, 4), System.Globalization.NumberStyles.HexNumber);
                        g = byte.Parse(htmlColorHex.Substring(4, 6), System.Globalization.NumberStyles.HexNumber);
                        b = byte.Parse(htmlColorHex.Substring(6, 8), System.Globalization.NumberStyles.HexNumber);
                        break;
                    case ColorEncoding.RGBA:
                        r = byte.Parse(htmlColorHex.Substring(0, 2), System.Globalization.NumberStyles.HexNumber);
                        g = byte.Parse(htmlColorHex.Substring(2, 4), System.Globalization.NumberStyles.HexNumber);
                        b = byte.Parse(htmlColorHex.Substring(4, 6), System.Globalization.NumberStyles.HexNumber);
                        a = byte.Parse(htmlColorHex.Substring(6, 8), System.Globalization.NumberStyles.HexNumber);
                        break;
                }
        ...
        public class QRCode
        {
            ...
            {
                return GetGraphic(pixelsPerModule, ColorConverter.FromHTML(darkColorHtmlHex), ColorConverter.FromHTML(lightColorHtmlHex));
            }
            ...
            public Texture2D GetGraphic(int pixelsPerModule, Color darkColor, Color lightColor)
            {
                ...
                Color[] darkColors = new Color[pixelsPerModule * pixelsPerModule];
                Color[] lightColors = new Color[pixelsPerModule * pixelsPerModule];
                for (int i = pixelsPerModule * pixelsPerModule - 1; i >= 0; i--)
                {
                    darkColors[i] = darkColor;
                ...
                for (int x = 0; x < size; x = x + pixelsPerModule)
                {
                    for (int y = 0; y < size; y = y + pixelsPerModule)
                    {
                        ...
                        if (module)
                        {
                            tex.SetPixels(x, y, pixelsPerModule, pixelsPerModule, darkColors);

                        }
                        else
                            tex.SetPixels(x, y, pixelsPerModule, pixelsPerModule, lightColors);
                    }
                }
                tex.Apply();
                return tex;
